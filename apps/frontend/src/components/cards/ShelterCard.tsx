...
const ShelterCard = (props: ShelterCardProps) => {
  ...
  const displayImage = isHome
    ? props.image
    : (props.logo ?? "https://placehold.co/200x200?text=Pas+de+logo");
      ...
      >
        <div className="h-72 overflow-hidden relative bg-gray-100">
          ...
          <img
            src={displayImage}
            alt={displayName}
            className={`w-full h-full object-cover transition-transform duration-700 group-hover:scale-105 ${!isHome ? "opacity-75" : ""}`}
          />
